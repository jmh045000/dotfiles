...
if [ -z "$fzf_exists" ] ; then
    ...
    ln -sfv ${HOME}/.fzf/bin/fzf ${HOME}/bin/
fi
...
ln -sfv "${PRGDIR}/vim/vimrc" ${HOME}/.vimrc
ln -sfv "${PRGDIR}/vim/gvimrc" ${HOME}/.gvimrc
ln -sfv "${PRGDIR}/vim/bundles.vim" ${HOME}/bundles.vim
mkdir -p ${HOME}/.vim
ln -sfv "${PRGDIR}/vim/ftplugin" ${HOME}/.vim/ftplugin
yes '' | vim +PluginInstall +qall
yes '' | vim +PluginUpdate +qall

# i3
mkdir -p ${HOME}/.config/i3
ln -sfv "${PRGDIR}/i3/config" ${HOME}/.config/i3/config
